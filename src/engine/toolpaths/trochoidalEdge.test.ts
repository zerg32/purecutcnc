...
function testCircularGuideClosesWithoutSeam(): void {
  const circle = Array.from({ length: 96 }, (_, index) => {
    const angle = 2 * Math.PI * index / 96
    return { x: 15 * Math.cos(angle), y: 15 * Math.sin(angle) }
  })
  const result = buildTrochoidalContour(circle, {
    orbitRadius: 1.5,
    advance: 0.7,
    toolDiameter: 3,
    angularDirection: -1,
  })
  assert(result.error === undefined, `unexpected circular-guide error ${result.error}`)
  const first = result.points[0]
  const last = result.points[result.points.length - 1]
  assert(approx(first.x, last.x) && approx(first.y, last.y), 'circular guide must close without a seam chord')
  assert(result.actualAdvance <= 0.7 + 1e-9, 'integer loop adjustment must not exceed requested advance')
}
