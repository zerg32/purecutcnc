...
function samplePosition(path: ArcLengthPath, distance: number): Point {
  ...
  const t = segmentLength > GEOMETRY_EPSILON ? (target - segmentStart) / segmentLength : 0
  return {
...
export function buildTrochoidalContour(
  ...
  // Clear an entry cavity before the orbit centre begins advancing.
